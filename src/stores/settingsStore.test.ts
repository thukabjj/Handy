...
describe("settingsStore", () => {
  ...
  describe("refreshAudioDevices", () => {
    ...
    it('filters out devices named "Default" or "default"', async () => {
      ...
      expect(
        state.audioDevices.find((d) => d.name === "Default" && d.index === "1"),
      ).toBeUndefined();
    });
  ...
  describe("updateSetting", () => {
    ...
    it("sets updating state during update", async () => {
      ...
      mockCommands.changeAudioFeedbackSetting.mockImplementation(async () => {
        updatingDuringCall = useSettingsStore
          .getState()
          .isUpdatingKey("audio_feedback");
      ...
      await act(async () => {
        await useSettingsStore
          .getState()
          .updateSetting("audio_feedback", false);
      });
    ...
    it("rolls back on error", async () => {
      ...
      await act(async () => {
        try {
          await useSettingsStore
            .getState()
            .updateSetting("push_to_talk", false);
        } catch {
          // Expected to throw
        }
      });
    ...
    it("rolls back binding on failure", async () => {
      ...
      await act(async () => {
        try {
          await useSettingsStore
            .getState()
            .updateBinding("shortcut", "Invalid");
        } catch {
          // Expected
        }
      });
  ...
  describe("getSetting", () => {
    it("returns setting value when settings exist", () => {
      ...
      expect(useSettingsStore.getState().getSetting("audio_feedback")).toBe(
        false,
      );
    });
  ...
  describe("post-process settings", () => {
    ...
    it("updatePostProcessApiKey clears cached models", async () => {
      ...
      await act(async () => {
        await useSettingsStore
          .getState()
          .updatePostProcessApiKey("openai", "new-key");
      });
  ...
  describe("initialize", () => {
    it("loads default settings, current settings, and checks custom sounds", async () => {
      ...
      mockCommands.checkCustomSounds.mockResolvedValueOnce({
        start: true,
        stop: false,
      });
