...
test.describe("Active Listening Settings", () => {
  ...
  test("should open the Active Listening feature panel from unified settings", async ({
    ...
    await expect(
      main.getByRole("heading", {
        name: "Enable Active Listening",
        exact: true,
      }),
    ).toBeVisible();
    await expect(
      main.getByText("Enable Active Listening to configure its settings."),
    ).toBeVisible();
  });
  ...
  test("should reveal advanced sections after enabling Active Listening", async ({
    ...
    await expect(
      main.getByText("AI provider is configured centrally"),
    ).toBeVisible();
    await expect(
  ...
  test("should expose prompt management after enabling Active Listening", async ({
    ...
    await expect(
      main.getByRole("heading", { name: "Prompts", exact: true }),
    ).toBeVisible();
    await expect(
      main.getByRole("button", { name: "New Prompt" }),
    ).toBeVisible();
  });
});
