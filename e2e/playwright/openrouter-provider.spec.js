...
test.describe('OpenRouter Provider Flow', () => {
  ...
  test('should configure shared AI settings in Cloud mode with OpenRouter', async ({
    ...
    await selectComboboxOption(page, page.getByTestId('shared-ai-mode'), 'Cloud');
    ...
    await expect(main.getByRole('heading', { name: 'Default Model', exact: true })).toBeVisible();
  ...
  test('should expose Ask AI shared-provider messaging after central AI configuration', async ({
    ...
    await expect(askAiMain.getByText('AI provider is configured centrally')).toBeVisible();
  ...
  test('should allow editing API keys for non-OpenRouter cloud providers', async ({
    ...
    const apiKeyInput = page.getByTestId('shared-ai-api-key').locator('input').first();
