...
test.describe('Handy Application', () => {
  ...
  test('should navigate between sidebar sections', async ({ page }) => {
    ...
    await expect(nav.locator('.bg-logo-primary\\/80')).toContainText('Settings');
    ...
    await expect(nav.locator('.bg-logo-primary\\/80')).toContainText('Transcribe');
  });
  ...
  test('should deep-link search results into the correct unified settings panel', async ({
    ...
    const nav = page.locator('nav[aria-label="Settings navigation"]');
    await expect(nav.locator('.bg-logo-primary\\/80')).toContainText('Settings');
    await expect(page.getByTestId('unified-section-features-toggle')).toHaveAttribute(
      'aria-expanded',
      'true',
    );
    await expect(page.getByTestId('feature-card-knowledge-base')).toBeVisible();
    await expect(page.locator('main[role="main"]').getByText('Knowledge Base')).toBeVisible();
  });

  test('should toggle debug mode with the keyboard shortcut', async ({ page }) => {
    const nav = page.locator('nav[aria-label="Settings navigation"]');
    const debugBefore = await nav.getByText('Debug', { exact: true }).isVisible().catch(() => false);

    await page.keyboard.press('Meta+Shift+KeyD');
    await page.waitForTimeout(400);
    const debugAfter = await nav.getByText('Debug', { exact: true }).isVisible().catch(() => false);
