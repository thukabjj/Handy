...
test.describe('Unified Settings', () => {
  ...
  test('should show the unified settings sections', async ({ page }) => {
    ...
    await expect(page.getByTestId('unified-section-audio-toggle')).toBeVisible();
    await expect(page.getByTestId('unified-section-output-toggle')).toBeVisible();
    await expect(page.getByTestId('unified-section-transcription-toggle')).toBeVisible();
    await expect(page.getByTestId('unified-section-app-toggle')).toBeVisible();
    await expect(page.getByTestId('unified-section-features-toggle')).toBeVisible();
  });

  test('should expose the feature cards inside advanced features', async ({ page }) => {
    await openSettings(page);
    ...
    await expect(page.getByTestId('feature-card-active-listening')).toBeVisible();
    await expect(page.getByTestId('feature-card-ask-ai')).toBeVisible();
    await expect(page.getByTestId('feature-card-batch-processing')).toBeVisible();
  ...
  test('should persist a feature checkbox after navigation', async ({ page }) => {
    await openSettings(page);
    await ensureUnifiedSectionExpanded(page, 'features');
    ...
    await openSettings(page);
    await ensureUnifiedSectionExpanded(page, 'features');
    await expect(page.getByTestId('feature-card-knowledge-base').locator('input[type="checkbox"]').first()).toHaveJSProperty(
      'checked',
      !initialState,
    );

    await page.getByTestId('feature-card-knowledge-base').locator('label').first().click();
    await page.waitForTimeout(200);
  });

  test('should expose interactive dropdowns in the audio section', async ({ page }) => {
    await openSettings(page);
